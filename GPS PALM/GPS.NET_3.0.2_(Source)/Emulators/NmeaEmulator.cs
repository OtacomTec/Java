...
namespace GeoFramework.Gps.Emulators
{
    public class NmeaEmulator : Emulator
    {
        ...
        public NmeaEmulator()
            : this("Generic NMEA-0183 Emulator (http://www.geoframeworks.com)")
        { }
        ...
        protected virtual Position EmulateError(Position truth)
        {
            // Calculate the error variance
            Distance error = Distance.FromMeters(
                (Seed.NextDouble() * _MeanDOP.Value) +
                DilutionOfPrecision.CurrentAverageDevicePrecision.ToMeters().Value);

            // Introduce the error
            return truth.TranslateTo(new Angle(Seed.NextDouble() * 360), error);
        }
